...
def implicit_midpoint_solver_ROM(y_0, right_hand_side, setup, t_vec, Nw=0, r_tol=1e-8, a_tol=1e-15, max_iter=100, windowing=True):
    """Solve the system
    ...
    for tt in range(1, len(t_vec)):
        ...
        if windowing:
            if t_vec[tt] in np.arange(T0 + int((Tf-T0)/Nw), Tf, int((Tf-T0)/Nw)) + setup.dt:
                ...
                Ur_e1_next = np.load("../data/ROM/" + str(setup.problem_dir) + "/SVD/basis_SVD_e1_" + str(setup.T0) + "_" + str(setup.T) + "_M_" + str(setup.M) + ".npy")[:, :setup.Nr]
                Ur_e2_next = np.load("../data/ROM/" + str(setup.problem_dir) + "/SVD/basis_SVD_e2_" + str(setup.T0) + "_" + str(setup.T) + "_M_" + str(setup.M) + ".npy")[:, :setup.Nr]
