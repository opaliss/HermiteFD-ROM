...
for M in range(3, 11):
    ...
    sol_midpoint_u = np.load("../data/FOM/two_stream/sample_"
                             + str(u_train[0]) + "/sol_FOM_u_"
                             + str(setup.Nv) + "_nu_" + str(setup.nu_e1)
    ...
    for kk in range(1, len(u_train)):
        ...
        new_data = np.load("../data/FOM/two_stream/sample_"
                           + str(u_train[kk])
                           + "/sol_FOM_u_" + str(setup.Nv)
                           + "_nu_" + str(setup.nu_e1)
                           + "_" + str(setup.T0) + "_" + str(setup.T)
                           + ".npy")[:, :3001]
    ...
    np.save("../data/ROM/two_stream/basis_SVD_e1_" + str(setup.T0) + "_30_M_" + str(M) + ".npy", U_e1[:, :1000])
    np.save("../data/ROM/two_stream/basis_SVD_e2_" + str(setup.T0) + "_30_M_" + str(M) + ".npy", U_e2[:, :1000])

    # singular values
    np.save("../data/ROM/two_stream/singular_values_SVD_e1_" + str(setup.T0) + "_30_M_" + str(M) + ".npy", S_e1)
    np.save("../data/ROM/two_stream/singular_values_SVD_e2_" + str(setup.T0) + "_30_M_" + str(M) + ".npy", S_e2)
