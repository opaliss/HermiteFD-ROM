...
if __name__ == "__main__":
    for u_e2 in [4, 4.1, 4.2, 4.3, 4.35, 4.4, 4.5, 4.6, 4.7]:
        setup = SimulationSetupTwoStreamFOM(Nx=150,
                                            Nv=200,
                                            epsilon=0.03,
                                            alpha_e1=np.sqrt(2),
        ...
        np.save("../data/FOM/bump_on_tail/sample_" + str(setup.u_e2) + "/sol_FOM_u_" + str(setup.Nv) + "_nu_" + str(
            setup.nu) + "_runtime_" + str(setup.T0) + "_" + str(setup.T), np.array([end_time_cpu, end_time_wall]))

        # save results
        np.save("../data/FOM/bump_on_tail/sample_" + str(setup.u_e2) + "/sol_FOM_u_" + str(setup.Nv) + "_nu_" + str(
            setup.nu) + "_" + str(setup.T0) + "_" + str(setup.T), sol_midpoint_u)
        np.save("../data/FOM/bump_on_tail/sample_" + str(setup.u_e2) + "/sol_FOM_t_" + str(setup.Nv) + "_nu_" + str(
            setup.nu) + "_" + str(setup.T0) + "_" + str(setup.T), setup.t_vec)
